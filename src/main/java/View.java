...
public class View {
  ...
  public void draw(GLAutoDrawable gla) {
    ...
    if (isRayTracing) {
      ...
      System.out.println("Ray Tracing draw");
      BufferedImage output = new BufferedImage(WINDOW_WIDTH, WINDOW_HEIGHT, BufferedImage.TYPE_INT_RGB);

      Color[][] tracedColor = scenegraph.raytrace(WINDOW_WIDTH, WINDOW_HEIGHT, modelView);

      for (int i = 0; i < tracedColor.length; i++) {
        for (int j = 0; j < tracedColor[i].length; j++) {
          Color color = tracedColor[i][j];
          output.setRGB(i, WINDOW_HEIGHT - j - 1, color.getRGB());
        }
      }
      //OutputStream outStream = null;
      File outputfile = new File("D://raytracing.jpg");
//      try {
//        outStream = new FileOutputStream("resources/raytracing.png");
